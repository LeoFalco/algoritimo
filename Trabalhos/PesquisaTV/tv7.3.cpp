...
#define n 1000 /*no maximo mil casas visitadas */
...
bool leiaSN(){ /*retorna true caso usuario digite s caso digite n retorna falso*/
	char var; 
	do{
		var = getch();  // le um caractere e guarda na variavel
		switch(var){
	...
	do{
		...
		if (leiaSN()){
			...
			if(stayInLoop){ 
				...
				gCasa[i].num = casa_num; /*atribuindo o numero a gCasa */
			}
		}else{
		...
		++i; /*fazendo o vetor andar*/
		printf("\n\n"); /*frescura */
	}while( i < n && stayInLoop );/* de 0 ate 999 enquanto stayInLoop estiver verdadeiro */
}
...
int encontreCasaMaisPovoada(){
	...
	for( int i = 0 ; i < n  ; i++){ /*percorrendo vetor */
		if( gCasa[i].teleEspec > gCasa[posicao].teleEspec){ /* se teleEspec > gCasa[posicao].teleEspec entao */
...
int encontreCanalMaisPopular(){
	...
	for( int i = 0 ; i < 4  ; i++){ /* percorrendo vetor de canais*/
		if(gCanal[i].espec > gCanal[posicao].espec){ /*se gCanal[i].espec > gCanal[posicao].espec entao  */
...
void pctgAudiencia(int publico_total){
	...
	for( int i = 0 ; i < 4  ; i++){ /*percorre o vetor de canais */
		/*formula da porcentagem*/                  /*typecast   abaixo*/
		gCanal[i].pctg_publico = (gCanal[i].espec / (float)publico_total)*100;
	}
}
